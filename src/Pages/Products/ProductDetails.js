...
function ProductDetails(props){
    ...
    useEffect(()=>{
        window.scrollTo(0,0)
        axios.get(`${URL_PATH}product/productdetail/${props.match.params.id}`)
        .then(res => {
                ...
                <ContainerBox>
                    <Container left>
                        <ProductImage>
                            ...
                            {data.images && data.images.slice(0,4).map((img, idx)=>
                                <img key = {idx} src={img} alt="product-img"/>)}
                            {show ? 
                            data.images && data.images.slice(4).map((img, idx)=>
                                <img key = {idx} src={img} alt="product-img" />) : null}
                            </div>
                            <SmallBtn onClick={()=>{
                                setShow(!show);
                                if(show){
                    ...
                    <Container>
                        <ProductText>
                            <div className = "product-name">
                                <div>{data.name}</div>
                                <div className="price">{data.price && data.price.toLocaleString()}</div>
                            </div>
                        ...
                        <div className = "btn">
                            ...
                            : <>
                            {addToCart && <FaCheck size="11" className="check-icon"/>}
                            {addToCart ? "장바구니에 추가되었습니다." : "구매하기"}
                            </>
                            }</Button>
                            <HeartBtn><img src={like} alt="wishlist" /></HeartBtn>
                        </div>
                        <SubText>
                            <div><img src={truck} className="truck-icon" alt="truck-icon"/><span className="circle">배송 가능</span></div>
                            <div><img src={container} className="container-icon" alt="container-icon"/><span className="circle">{data.product_location}에 재고가 있습니다.</span></div>
                            <div><img src={heart} className="heart-icon" alt="heart-icon"/><span>마음이 바뀌어도 괜찮아요. 365일 이내에 영수증과 온전한 상태의 제품을 가져오시면 전액 환불해드립니다.</span></div>
                        </SubText>
                    </Container>
                </ContainerBox>
            </div>
        ...
        <Modal activeTab={activeTab}>
            <div className={`modal ${activeTab === null ? "slide-close" : ""}`} >
                <div className = "close-btn">
                    <span className="close-icon" onClick={() => setActiveTab(null)}><IoMdClose size="22" color="#111"/></span>
                </div>
